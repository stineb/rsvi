...
```{r}
df_homo <- read_csv2("./data/sites_v2.csv")
...
ddf <- ddf %>% add_scaled_rsvi(dovars, method="range")
```
## Complement data
...
## Rearrange data. 
Function returns list( df_dday, df_dday_aggbydday )
...
df_dday_agg_inst %>% 
  ...
  geom_line() +
  labs(x="Day after drought onset", y="Unitless") + ggtitle("US-SRM") +
...
ddf %>% 
  ...
  theme_bw() + facet_grid(classid~is_flue_drought, labeller = labeller(is_flue_drought = as_labeller(day))) +
  labs(x="NDVI", y="CCI") +
  geom_point(alpha=0.2) + scale_color_brewer(palette="Dark2")

ddf %>% 
  filter(!is.na(is_flue_drought)) %>% 
  ggplot(aes(x=ndvi, y=pri, color=is_flue_drought)) +
  scale_colour_discrete(name  = NULL) +
  theme_bw() + facet_grid(classid~is_flue_drought, labeller = labeller(is_flue_drought = as_labeller(day))) +
  labs(x="NDVI", y="PRI") +
  geom_point(alpha=0.2) + scale_color_brewer(palette="Dark2")
...
ddf %>% 
  filter(!is.na(is_flue_drought)) %>% 
  ggplot(aes(x=ndvi, y=NIRv, color=is_flue_drought)) +
  scale_colour_discrete(name = NULL) +
  theme_bw() + facet_grid(classid~is_flue_drought, labeller = labeller(is_flue_drought = as_labeller(day))) +
  labs(x="NDVI", y="NIRv") +
  geom_point(alpha=0.2) + scale_color_brewer(palette="Dark2")
...
ddf %>% 
  filter(!is.na(is_flue_drought)) %>% 
  ggplot(aes(x=ndvi, y=evi, color=is_flue_drought)) +
  scale_colour_discrete(name  = NULL) +
  theme_bw() + facet_grid(classid~is_flue_drought, labeller = labeller(is_flue_drought = as_labeller(day))) +
  labs(x="NDVI", y="EVI") +
  geom_point(alpha=0.2) + scale_color_brewer(palette="Dark2")
...
- Cluster with meteorological variables
```{r}
# ddf %>% 
#   filter(!is.na(is_flue_drought)) %>% 
#   ggplot(aes(x=ndvi, y=pri, color=is_flue_drought)) +
...
#   geom_point(alpha=0.2) + scale_color_brewer(palette="Dark2") + theme_bw()
```

## Index vs Index: pdf saved, it has problems here
```{r}
...
```{r}
# Filtered sites: final selection (19)
...
# for(i in 1:length(unique(sitios))){
...
print(ggplot(ts, aes(x=date, y=value)) +
    ...
    color=NA) +
  geom_line() + theme_classic() + ggtitle(paste(sitename,class,sep=" - ")) +
  scale_x_date(limits = as.Date(c('2000-01-01','2015-01-01'))) + labs(x="Date"))
}else{
 print(ggplot(ts, aes(x=date, y=value)) +
    facet_grid(rows=vars(variables), scales = "free_y")  +
...
#   summarise(count=n()) %>% 
#   ggplot(aes(x=cluster, y=count, fill=is_flue_drought)) + theme_bw() +
#   geom_bar(stat="identity", position=position_dodge()) +  scale_fill_brewer(palette="Dark2")
``` 
K-means clustering with 2 clusters of sizes 12556, 4104
Within cluster sum of squares by cluster:
[1] 2042414851 2163736469
...
[1] 2042414851 2163736469
 (between_SS / total_SS =  67.9 %)
  ...
  - Random-forest, supervised machine-learning with `is_flue_drought` as target and RSVIs (plus other meteorological variables) as predictors. The `caret` R package offers a nice framework to use machine-learning models. 
  
## RANDOM FOREST and NEURAL NETWORKS
```{r}

# source(for_randomForest_Categorical_bs_pc.R)
