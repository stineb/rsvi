...
ddf_cluster <- ddf %>%
  # filter(temp>5) %>%
  select(is_flue_drought, one_of(dovars)) %>% # Add temperature and PPFD here if you like. (Paula: Didn't change the results)
  drop_na()
...
```  
Adding temperature and PPFD in the mix of variables used for K-means but the proportions didn't in each cluster change
```{r}
ddf_cluster <- ddf %>%
  # filter(temp>5) %>%
  select(is_flue_drought, one_of(dovars), temp, prec_fluxnet2015, ppfd_fluxnet2015) %>% 
  drop_na()
outkmeans <- kmeans( ddf_cluster, 2 )
ddf_cluster %>% 
  mutate(cluster=as.factor(outkmeans$cluster-1)) %>%
