...
df_homo$sitename
```
## sPRI: To ensure positive values that are better comparable standardized as by Rahman et al. and Goerner et al.(Vicca 2016)
```{r}
# ddf <- ddf %>% 
#   mutate(pri = (pri+1)/2)
```
...
ddf_cluster <- ddf %>%
  # filter(temp>5) %>%
  select(is_flue_drought, one_of(dovars), temp, prec_fluxnet2015, ppfd_fluxnet2015) %>% # Add temperature and PPFD here if you like. (Paula: Didn't change the results)
  drop_na()
