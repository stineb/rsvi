...
```
## sPRI:
 To ensure positive values that are better comparable standardized as by Rahman et al.2004 Vicca et al 2016.
```{r}
# ddf <- ddf %>% 
#   mutate(pri = (pri+1)/2)
```
...
namesite <- unique(ddf$site)[i]
ddfsite <- ddf %>% filter(site == namesite)
...
out_align$df_dday_agg_inst %>% filter(site == namesite) %>%
  ggplot(aes(x=dday, y=pri_median)) +
  geom_line() 

##  Raw data, aligned by drought events
out_align$df_dday %>% #filter(site == namesite) %>%
